...
def col2img(input, height_pad, width_pad, kernel_size, channel, padding, stride):
    ...
    for idxh in range(height):
        for idxw in range(width):
            ...
            idxw * stride: idxw * stride + kernel_size] += input[:, :, :, idxh * width + idxw].reshape(input.shape[0],
                                                                                                       channel,
                                                                                                       kernel_size, -1)
...
class ConvolutionalLayer(object):
    ...
    def backward_speedup(self, top_diff):
        ...
        bottom_diff = col2img(bottom_diff_col, height_pad, width_pad, self.kernel_size, self.channel_in, self.padding,
                              self.stride)
...
class MaxPoolingLayer(object):
    ...
    def backward_speedup(self, top_diff):
        ...
        bottom_diff = col2img(pool_diff, self.input.shape[2], self.input.shape[3], self.kernel_size,
                              self.input.shape[1], 0, self.stride)
